...
{{< note >}}These instructions reference a sample Listserv called `TESTLIST@listserv.gsa.gov`. Just replace `TESTLIST` in the command line examples above with the name of YOUR Listserv to apply them to your Listserv. For example, use `set CONTENT-MANAGERS-L digest` to receive a daily digest of plain text posts from the Web Managers Listserv.{{< /note >}}
