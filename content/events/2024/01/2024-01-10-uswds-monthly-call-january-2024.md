...
**Slide 50:** The audit has been an iterative process that’s evolved over time. We gained inspiration from other design systems to determine what we felt would best benefit our community. From that starting point, we moved into using the [W3 Quick reference resource](https://www.w3.org/WAI/WCAG22/quickref/?versions=2.1#top) as a testing guide (we are adding that link in the chat). As I mentioned earlier, there are more than 78 potential success criteria to check against, so the W3 quick reference provided us the means by which we, hopefully, didn’t overlook specific success criteria for a given component.
