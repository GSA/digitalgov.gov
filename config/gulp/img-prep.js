...
function fileStaticTidy(done) {
  const workingDirectory = "./content/images/_inbox/";  ///content/images/_inbox/2014-anle-agreement.pdf
  const origDirectory = "./content/images/_working/originals";
  const toProcessDirectory = "./content/images/_working/to-process";
  const extAllowed = [".pdf", ".doc", ".docx", ".ppt", ".pptx", ".xls", ".xlsx"];
  var newfileName = "";
...
function writeDataStaticFile() {
  return (
    src("content/images/_working/to-process/*.pdf")
    // src("content/images/_working/to-process/*.{pdf,doc,docx,ppt,pptx,xls,xlsx}")
      // write the .yml file for this image
      .pipe(
        tap(function foo(file) {
          var uid = file.path.match(/([^\/]+)(?=\.\w+$)/g); // gets the slug/filename from the path
          ...
          var file_data = [
            ...
            "# https://s3.amazonaws.com/digitalgov/" +
              ...
              format +
              "\n",
            '# File shortcode: {{< asset-static file="' + uid + '.' + extension + '" label="View the slides (PDF, 5.95 MB, 52 pages)" >}}\n',
            "date     : " + get_curr_date(),
            "uid      : " + uid,
            // "width    : " + dimensions.width,
            // "height   : " + dimensions.height,
